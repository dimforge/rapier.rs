...
to compute contacts points and generate [collision events](#collision-events).

All the pairs detected by the broad-phase can be accessed from the physics `World` class.

## Collision events
The narrow-phase can generate [collision events](#contact-events) between two colliders.
...
:::warning
Collision events are only generated between two colliders  if at least one of them has the
`ActiveEvents.COLLISION_EVENTS` flag in its [active events](colliders#active-events).
:::

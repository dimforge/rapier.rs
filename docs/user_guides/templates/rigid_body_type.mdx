...
There are three types of rigid-bodies, identified by the <rapier>`RigidBodyType` enumeration</rapier><bevy>`RigidBody` component</bevy><js>`RigidBodyType` enumeration</js>:
...
account needs to be done manually either by using [scene queries](scene_queries) to detect nearby obstacles, or by using the built-in

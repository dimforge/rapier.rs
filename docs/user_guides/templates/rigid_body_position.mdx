...
number (in 2D). <notjs>Both are combined into an isometry</notjs><bevy> stored in the `RigidBodyPositionComponent` component</bevy>.
...
fn modify_body_translation(mut positions: Query<&mut RigidBodyPositionComponent>) {
...
For **velocity-based** kinematic bodies, it is recommended to set its velocity instead of setting its position directly.
For **position-based** kinematic bodies, it is recommended to modify the `next_position` field of the `RigidBodyPositionComponent` component instead

...
(by changing the <notjs>`IntegrationParameters::max_ccd_substeps`</notjs><js>`IntegrationParameters.maxCcdSubsteps`</js> field).
...
fn modify_body_ccd(mut ccds: Query<&mut RigidBodyCcdComponent>) {

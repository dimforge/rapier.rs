...
sprite: <rapier>`ColliderBuilder::cuboid(50.0, 50.0)`</rapier><bevy>`ColliderShape::cuboid(50.0, 50.0)`</bevy><js>`ColliderDesc.cuboid(50.0, 50.0)`</js> (we set `50.0`
...
tall and 100 meters wide, which is huge. Therefore it is recommended ot have a scaling factor between the graphics
...
In this example, we could set the `RapierConfiguration::scale` field to `50.0` so that the translational part of 
all the positions synchronized by the [`ColliderPositionSync`](colliders#sync-with-bevy-transform) and
[`RigidBodyPositionSync`](rigid_bodies#sync-with-bevy-transform) are automatically multiplied by `50.0` when
they are written into the `Transform` component.

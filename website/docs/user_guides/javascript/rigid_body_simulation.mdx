...
This `World` class is responsible for creating/deleting rigid bodies and joints, advancing the simulation, retrieving
...
import('@dimforge/rapier2d').then(RAPIER => {
  ...
  rigidBodyDesc.setTranslation(0.0, 1.0);
  ...
  while (true) {
      ...
      console.log("Rigid-body position: ", position);
  }
})
```
...
:::note
Unfortunately, the JS bindings don't expose a nice builder pattern similar to what we can do with the `RigidBodyBuilder`
in Rust. This is due to some limitations on the Rust WASM/JS interface which prevents us from doing this efficiently. This
...
can then be passed to the `rigidBody.createCollider(desc)` function to create the actual collider and attach it to
the given `rigidBody`:
...
import('@dimforge/rapier2d').then(RAPIER => {
  ...
  colliderDesc.setDensity(2.0); // The default density is 1.0.
...
import('@dimforge/rapier2d').then(RAPIER => {
    ...
    let force = new RAPIER.Vector(1.0, 2.0);
    let impulse = new RAPIER.Vector(0.1, 0.2);
    let torque = 3.0;
...
import('@dimforge/rapier3d').then(RAPIER => {
    // ... World initialization.
    let force = new RAPIER.Vector(1.0, 2.0, 3.0);
    let impulse = new RAPIER.Vector(0.1, 0.2, 0.3);
    let torque = new RAPIER.Vector(0.0, 3.0, 0.0);
    let torqueImpulse = new RAPIER.Vector(0.0, 0.3, 0.0);

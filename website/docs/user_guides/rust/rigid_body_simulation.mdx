...
sensors. Those colliders are attached to [rigid-bodies](#rigid-bodies) which are responsible for the simulation of
...
fn main() {
    ...
    let world = World::new(Vector2::new(0.0, -9.81));
...
use rapier3d::world::World;

fn main() {
    // Here the gravity is -9.81 along the y axis.
    let world = World::new(Vector3::new(0.0, -9.81, 0.0));
    let mut bodies = RigidBodySet::new();
    let mut colliders = ColliderSet::new();
    let mut joints = JointSet::new();
    // We ignore contact events for now.
...
The **world** is responsible for executing the whole simulation: collision detection as well as the simulation of
physical phenomena: gravity, contact forces, joints, etc. This world will affect:

- The `RigidBodySet` which is a set rigid-bodies.
- The `ColliderSet` which is a set of colliders.
- The `JointSet` which is a set of joint.

It will also use an event-handler for handling events happening during the simulation.

All those sets associate a unique handle to each element it contains. It is this handle that the physicsworld
will end up storing on their internal data structure. That way, the world doesn't own the elements itself, which is very
...
### Body statuses
Any body can have one of three different statuses identified by the [`dynamics::BodyStatus`](/rustdoc/rapier3d/dynamics/enum.BodyStatus.html)
enumeration:
...
* **`BodyStatus::Static`:** Indicates the body cannot move. It acts as if it has an infinite mass and will not be
    ...
    ones. This is typically used for the ground or for temporarily freezing a body.
* **`BodyStatus::Kinematic`:** Indicates the body velocity must not be altered by the physics engine. The user is
    ...
    used for moving **platforms**, **elevators**, etc.
...
let _ = ColliderBuilder::capsule_y(0.5);
...
let collider = ColliderBuilder::new(Shape::Ball(Ball::new(0.5))
    ...
    // It will be attached to the body with handle `parent_body_handle`.
    .build(parent_body_handle);
```
...
`world.step(...)` method. Typically, a game loop will look like this:
...
loop {
    ...
    handle_physics_events(&event_collector);
    render_scene();
}
```
...
of all bodies. Forces taken into account by **rapier** includes gravity, contact forces, and joints forces.
...
using `world.set_timestep(...)`. Note that if you use the [International System of Units](https://en.wikipedia.org/wiki/International_System_of_Units)
(SI units), this timestep is to be given in seconds. Keep in mind that the timestep length strongly affects the accuracy
of the simulation: the smaller the timestep, the more accurate the simulation will be.

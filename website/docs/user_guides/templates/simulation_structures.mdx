...
shape-casting (sweep tests), intersection tests, on all the colliders of the scene. Before it is used, the
`QueryPipeline` needs to be updated in order to take the new colliders' positions into account.
...
## Joint set
The `JointSet` contains all the **joints** that needs to be simulated. This set is represented as a
generational-arena, i.e., a vector where each element is indexed using a __handle__ that combines
...
[the dedicated section](advanced_collision_detection#contact-and-intersection-events).
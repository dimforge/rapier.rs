...
of the `ImpulseJointSet` resource (instead of being actual components). This will be improved in the future.
:::
...
```rust
let joint = FixedJoint::new().local_anchor(point![0.0, -2.0]);
...
```rust
let joint = FixedJoint::new().local_anchor(point![0.0, 0.0, -2.0]);
joint_set.insert(&mut rigid_body_set, body_handle1, body_handle2, joint);
```
...
:::note
In 2D, revolute joints are spherical joints are the same thing. Therefore, there is no `SphericalJoint` type in
...
```rust
let x = Vector::x_axis();
let mut joint = PrismaticJoint::new(x)
    .local_anchor1(point![0.0, 1.0])
    .local_anchor2(point![0.0, -3.0])
    .limit_axis([-2.0, 5.0]);
joint_set.insert(&mut rigid_body_set, body_handle1, body_handle2, joint);

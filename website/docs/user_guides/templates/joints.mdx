...
```js
let params = JointParams.fixed({ x: 0.0, y: 0.0 }, 0.0, { x: 0.0, y: -2.0 }, 0.0);
let joint = world.createJoint(params, bodyHandle1, bodyHandle2);
```
...
```js
let desc = JointDesc.ball({ x: 0.0, y: 0.0 }, { x: 0.0, y: -3.0 });
let joint = world.createJoint(desc, bodyHandle1, bodyHandle2);
```
...
```js
let desc = JointDesc.ball({ x: 0.0, y: 0.0, z: 0.0 }, { x: 0.0, y: -3.0, z: 0.0 });
let joint = world.createJoint(desc, bodyHandle1, bodyHandle2);
```
...
let desc = JointDesc.revolute({ x: 0.0, y: 0.0, z: 1.0 }, x, { x: 0.0, y. 0.0, z: -3.0 }, x);
let joint = world.createJoint(desc, bodyHandle1, bodyHandle2);
```
...
```js
let x = { x: 1.0, y: 0.0 };
let desc = JointDesc.prismatic({ x: 0.0, y: 0.0 }, x, { x: 0.0, y: -3.0 }, x);
...
let desc = JointDesc.prismatic({ x: 0.0, y: 0.0, z: 1.0 }, x, z, { x: 0.0, y: 0.0, z: -3.0 }, x, z);
desc.limitsEnabled = true;
desc.limits = [-2.0, 5.0];
let joint = world.createJoint(desc, bodyHandle1, bodyHandle2);
```

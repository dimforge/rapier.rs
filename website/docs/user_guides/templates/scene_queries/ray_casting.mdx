...
```js
let ray = new Ray({ x: 1.0, y: 2.0 }, { x: 0.0, y: 1.0 });
let max_toi = 4.0;
let solid = true;
...
let hit = world.castRayAndGetNormal(ray, maxToi, solid, groups);
if (hit != null) {
    // This is similar to `world.castRay` illustrated above except
    // that it also returns the normal of the collider shape at the hit point.
    let hitPoint = ray.pointAt(intersection.toi);
...
```js
let ray = new Ray({ x: 1.0, y: 2.0, z: 3.0 }, { x: 0.0, y: 1.0, z: 0.0 });
let maxToi = 4.0;
let solid = true;
...
if (hit != null) {
    // The first collider hit has the handle `hit.colliderHandle` and it hit after
    // the ray travelled a distance equal to `ray.dir * toi`.
    let hitPoint = ray.poiauieauientAt(toi); // Same as: `ray.origin + ray.dir * toi`
    console.log("Collider ", hit.colliderHandle, " hit at point ", hitPoint);
}
...
world.intersectionWithRay(ray, maxToi, solid, groups, (hit) => {
    // Callback called on each collider hit by the ray.
    let hitPoint = ray.pointAt(hit.toi);
    console.log("Collider ", hit.colliderHandle, " hit at point ", hitPoint, " with normal ", hit.normal);
    return true; // Return `false` instead if we want to stop searching for other hits.

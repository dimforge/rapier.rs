...
let collider = ColliderBundle {
    shape: ColliderShape::cuboid(1.0, 2.0),
...
let collider = ColliderBundle {
    shape: ColliderShape::cuboid(1.0, 2.0, 3.0),
    collider_type: ColliderType::Sensor,
...
/* Set the restitution coefficient and restitution combine rule
   inside of a system. */
fn modify_collider_restitution(mut materials: Query<&mut ColliderMaterial>) {
    for mut co_material in materials.iter_mut() {
        co_material.restitution = 0.7;
        co_material.restitution_combine_rule = CoefficientCombineRule::Min;
    }
}
```
...
fn modify_collider_groups(mut groups: Query<&mut ColliderGroups>) {
    for mut co_groups in groups.iter_mut() {
...
by setting its corresponding `active_event` bit to `1`:
- Setting the `ActiveEvents::FILTER_CONTACT_PAIRS` bit to 1 enables the
  [contact events](advanced_collision_detection#contact-events) involving the
  collider.
- Setting the `ActiveEvents::FILTER_INTERSECTION_PAIRS` bit to 1 enables the manual
  [intersection events](advanced_collision_detection#intersection-events) involving the
  collider.
...
- Setting the `ActiveHooks::FILTER_CONTACT_PAIRS` bit to 1 enables the manual
  [filtering of all the contact pairs](advanced_collision_detection#contact-and-intersection-filtering) involving the
  collider.
- Setting the `ActiveHooks::FILTER_INTERSECTION_PAIRS` bit to 1 enables the manual
  [filtering of all the contact pairs](advanced_collision_detection#contact-and-intersection-filtering) involving the
  collider.
- Setting the `ActiveHooks::MODIFY_SOLVER_CONTACTS` bit to 1 enables the manual

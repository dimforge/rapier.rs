...
to compute contacts points and generate [contact or intersection events](#contact-and-intersection-events).
...
or [intersection events](#intersection-events) between one sensor collider and another collider.

There events can be collected by optionally passing an instance of `EventQueue` to `World.step(eventQueue)`.
The colliders involved in each event is identified by its integer handle.
...
Intersection events are triggered when two colliders (where at least one of them is a sensor) transition between
intersecting or not. Contact events are triggered when two non-sensor colliders transition between having zero contact
points and having at least one contact point. Transitioning between having one contact point to having more than one
contact points is not reported an event.

:::warning
Intersection (resp. contact) events are only generated between two colliders (where at least one of the is a sensor) if at
least one of them has the `ActiveEvents.INTERSECTION_EVENTS` (resp. `ActiveEvents.CONTACT_EVENTS`) flag in its
[active events](colliders#active-events).
:::
...
```js
world.contactsWith(colliderHandle, (otherColliderHandle) => {
    // This closure is called on the handle of each collider potentially
    // in contact with `colliderHandle`.
});
...
```js
world.intersectionsWith(colliderHandle, (otherColliderHandle) => {
    // This closure is called on the handle of each collider potentially
    // intersecting the collider with handle `colliderHandle`.
});

let intersections = world.intersectionPair(handle1, handle2);
```
...
[collision groups and solver groups](colliders#collision-groups-and-solver-groups)).
...
- For each potential contact pair (between two non-sensor colliders) detected by the broad-phase, if at least one
  ...
  [active hooks](colliders#active-hooks), then `PhysicsHooks.firterContactPair` will be called. If this filter

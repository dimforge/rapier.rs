...
let rigid_body = RigidBodyBundle {
    ...
    velocity: RigidBodyVelocity { linvel: [1.0, 2.0].into(), angvel: 0.2 },
    forces: RigidBodyForces { gravity_scale: 0.5, ..Default::default() },
    activation: RigidBodyActivation { can_sleep: false, ..Default::default() },
...
let rigid_body = RigidBodyBundle {
    ...
    velocity: RigidBodyVelocity { linvel: [1.0, 2.0, 3.0].into(), angvel: [0.2, 0.0, 0.0].into() },
    forces: RigidBodyForces { gravity_scale: 0.5, ..Default::default() },
    activation: RigidBodyActivation { can_sleep: false, ..Default::default() },
    ccd: RigidBodyCcd { ccd_enabled: true, ..Default::default() },
    ..Default::default()
...
fn modify_body_translation(mut positions: Query<&mut RigidBodyPosition>) {
    for rb_pos in positions.iter_mut() {
...
For kinematic bodies, it is recommended to modify the `next_position` field of the `RigidBodyPosition` component instead
...
fn modify_body_velocity(mut velocities: Query<&mut RigidBodyVelocity>) {
    for rb_vel in velocities.iter_mut() {
...
fn modify_body_gravity_scale(mut forces: Query<&mut RigidBodyForces>) {
    for rb_forces in forces.iter_mut() {
...
fn apply_forces(mut rigid_bodies: Query<(&mut RigidBodyForces, &mut RigidBodyVelocity, &RigidBodyMassProps)>) {
    for (rb_forces, rb_vel, rb_mprops) in rigid_bodies.iter_mut() {
        // Apply forces.
...
let collider = ColliderBundle {
    ...
    // The default density is 1.0, we are setting 2.0 for this example.
    mprops: ColliderMassProperties::Density(2.0),
    ..Default::default()
}

// When the collider is attached, the rigid-body's mass and angular
// inertia will be automatically updated to take the collider into account.
command.spawn_bundle(rigid_body)
...
let rigid_body = RigidBodyBundle {
    mprops: MassProperties::new(point![1.0, 2.0], 10.0, 0.5).into(),
    ..Default::default()
};
commands.spawn_bundle(rigid_body);
```
...
fn modify_body_mass_props(mut mprops: Query<&mut RigidBodyMassProps>) {
    for rb_mprops in mprops.iter_mut() {
...
/* Set the mass properties when the rigid-body bundle is created. */
let rigid_body = RigidBodyBundle {
    mprops: MassProperties::new(point![1.0, 2.0, 0.0], 10.0, vector![0.3, 0.4, 0.5]).into(),
    ..Default::default()
};
commands.spawn_bundle(rigid_body);
```
...
let rigid_body = RigidBodyBundle {
    mprops: RigidBodyMassPropsFlags::TRANSLATION_LOCKED | RigidBodyMassPropsFlags::ROTATION_LOCKED_X,
    ..Default::default()
};
commands.spawn_bundle(rigid_body);
```
...
/* Lock translations and/or rotations when the rigid-body bundle is created. */
let rigid_body = RigidBodyBundle {
    mprops: RigidBodyMassPropsFlags::TRANSLATION_LOCKED | RigidBodyMassPropsFlags::ROTATION_LOCKED,
    ..Default::default()
};
commands.spawn_bundle(rigid_body);
```
...
fn modify_body_locked_flags(mut dampings: Query<&mut RigidBodyDamping>) {
    for rb_damping in dampings.iter_mut() {
...
originating from contacts with some other bodies. For example this can be used ot model a player represented as a
...
fn modify_body_locked_flags(mut dominances: Query<&mut RigidBodyDominance>) {
    for rb_dominance in dominances.iter_mut() {
        rb_dominance.0 = 10;
    }
}
```
...
let rigid_body = RigidBodyBundle {
    dominance: RigidBodyCcd { ccd_enabled: true, ..Default::default() },
    ..Default::default()
};
commands.spawn_bundle(rigid_body);
```
...
fn modify_body_locked_flags(mut ccds: Query<&mut RigidBodyCcd>) {
    for rb_ccd in ccds.iter_mut() {
